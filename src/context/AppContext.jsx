...
// Centralised localStorage writer — prevents scattered setItem calls
...
// Sanitise imported profile: clamp indices, ignore unknown keys
...
// Darken a hex colour by a fixed amount (used for light theme accent contrast)
...
export function AppProvider({ children }) {
  ...
  const [profile, setProfileState] = useState(() => {
    try {
      return sanitiseProfile({ ...DEFAULT_PROFILE, ...JSON.parse(localStorage.getItem("cl-profile") || "{}") });
    } catch {
  ...
  const [favourites, setFavourites] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("cl-favs") || "[]");
    } catch {
      return [];
    }
  });
  const [history, setHistory] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("cl-history") || "[]");
    } catch {
      return [];
    }
  });
  const [notifications, setNotifications] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("cl-notifs") || "null") || NOTIFS_INIT;
    } catch {
      return NOTIFS_INIT;
    }
  });
  ...
  // Effect 1: apply theme + accent palette to CSS variables
  useEffect(() => {
    const root = document.documentElement;
    root.setAttribute("data-theme", theme);
    persist("cl-theme", theme);
  ...
  // Effect 2: apply font family + size (independent of theme/palette)
  useEffect(() => {
  ...
  useEffect(() => {
    persist("cl-sidebar", sidebarOpen);
  }, [sidebarOpen]);
  ...
  // Sanitise and apply imported profile
    ...
    (file) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (data.profile) saveProfile(sanitiseProfile(data.profile));
          if (data.theme && (data.theme === "dark" || data.theme === "light")) {
            setThemeState(data.theme);
            persist("cl-theme", data.theme);
          }
          if (Array.isArray(data.favourites)) {
            setFavourites(data.favourites);
            persist("cl-favs", data.favourites);
          }
