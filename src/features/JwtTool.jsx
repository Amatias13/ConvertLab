    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      ...
      <Panels>
        <Panel0 style={{ maxWidth: 340 }}>
          <PanelLabel right={status && <StatusBadge type={statusType}>{status}</StatusBadge>}>JWT Token</PanelLabel>
          <CodeArea value={input} onChange={setInput} placeholder={"Paste JWT token...\n\neyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."} />
        </Panel0>

        <Panel>
          <PanelLabel>Decoded</PanelLabel>
          <div style={{ flex: 1, overflow: "auto", padding: "1rem", display: "flex", flexDirection: "column", gap: 12 }}>
            {!decoded && <div style={{ fontSize: 12, color: "var(--text3)" }}>Paste a JWT token to decode it</div>}
            {decoded && (
              <>
                {[
                  ...
                  return (
                    <div key={key} style={{ borderRadius: 10, border: "1px solid var(--border)", overflow: "hidden" }}>
                      ...
                      <pre style={{ padding: "0.85rem", fontFamily: "var(--mono)", fontSize: 12, lineHeight: 1.7, background: "var(--bg2)", overflowX: "auto" }}>{JSON.stringify(data, null, 2)}</pre>
                    </div>
                  );
                })}
                <div style={{ borderRadius: 10, border: "1px solid var(--border)", overflow: "hidden" }}>
                  ...
                  <pre style={{ padding: "0.85rem", fontFamily: "var(--mono)", fontSize: 12, lineHeight: 1.7, background: "var(--bg2)", wordBreak: "break-all", whiteSpace: "pre-wrap", color: "var(--text3)" }}>{decoded.sig}</pre>
                </div>
              </>
            )}
          </div>
