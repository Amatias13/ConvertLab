  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      ...
      <div style={{ padding: "1.25rem", borderBottom: "1px solid var(--border)", display: "flex", gap: "1rem", alignItems: "center", flexWrap: "wrap" }}>
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Enter a number..." style={{ fontFamily: "var(--mono)", fontSize: "1.4rem", flex: 1, minWidth: 180 }} />
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
          <OptLabel>Decimals:</OptLabel>
          <OptGroup>
            {[0, 1, 2, 3, 4].map((v) => (
              <OptBtn key={v} active={decimals === v} onClick={() => setDecimals(v)}>
                {v}
              </OptBtn>
            ))}
          </OptGroup>
        ...
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
          ...
          <OptGroup>
            <OptBtn active={style === "decimal"} onClick={() => setStyle("decimal")}>
              Number
            </OptBtn>
            <OptBtn active={style === "currency"} onClick={() => setStyle("currency")}>
              Currency (€)
            </OptBtn>
            <OptBtn active={style === "percent"} onClick={() => setStyle("percent")}>
              Percent
            </OptBtn>
          </OptGroup>
        </div>
