...
export default function MarkdownTool({ showToast }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      <ToolHeader title="Markdown Preview" desc="Real-time Markdown rendering">
        <Btn onClick={() => { navigator.clipboard.writeText(input); showToast('Copied') }}>Copy source</Btn>
      </ToolHeader>
      ...
      <Panels>
        ...
        <Panel>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '1.5rem' }}>
            <style>{PREVIEW_STYLE.split('\n').map(l => l.trim()).join(' ')}</style>
