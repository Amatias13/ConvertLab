...
export default function Base64Tool() {
  ...
  const encode = () => {
    try {
      ...
      showToast("Encoding error: " + e.message);
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
      ...
      <OptionsBar>
        ...
        <OptGroup>
          <OptBtn active={!urlSafe} onClick={() => setUrlSafe(false)}>
            Standard
          </OptBtn>
          <OptBtn active={urlSafe} onClick={() => setUrlSafe(true)}>
            URL-safe
          </OptBtn>
        </OptGroup>
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, fontSize: 11, color: "var(--text3)", alignItems: "center" }}>
          <span>
            In: <b style={{ color: "var(--text2)", fontFamily: "var(--mono)" }}>{inSize}b</b>
          </span>
          <span>
            Out: <b style={{ color: "var(--text2)", fontFamily: "var(--mono)" }}>{outSize}b</b>
          </span>
          <span>
            Ratio: <b style={{ color: "var(--text2)", fontFamily: "var(--mono)" }}>{ratio}</b>
          </span>
          <Btn
            onClick={() => {
              copy(encoded, "Copied");
