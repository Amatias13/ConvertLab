...
export default function UrlTool({ showToast }) {
  ...
  const decode = () => {
    try { setInput(decodeURIComponent(output)) }
    catch { showToast('Invalid encoded URL') }
  }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      <ToolHeader title="URL Encoder / Decoder" desc="Encode/decode URLs and parse query strings">
        <Btn onClick={() => setOutput(encodeURIComponent(input))} primary>Encode →</Btn>
