...
const SQL_CLAUSES = ['SELECT', 'FROM', 'WHERE', 'LEFT JOIN', 'RIGHT JOIN', 'INNER JOIN', 'OUTER JOIN', 'FULL JOIN', 'JOIN', 'GROUP BY', 'ORDER BY', 'HAVING', 'LIMIT', 'OFFSET', 'UNION ALL', 'UNION', 'INSERT INTO', 'VALUES', 'SET', 'ON']
...
function formatSQL(sql, indent = 2) {
  ...
  let result = sql.trim()
  KW_REGEXES.forEach(([re, kw]) => { result = result.replace(re, kw) })
  CLS_REGEXES.forEach(([re, replacement]) => { result = result.replace(re, replacement) })
...
export default function SqlTool({ showToast }) {
