...
function Section({ title, children }) {
  return (
    <div style={{ marginBottom: "1.5rem" }}>
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--text3)", marginBottom: "0.75rem", paddingBottom: "0.4rem", borderBottom: "1px solid var(--border)" }}>{title}</div>
      {children}
    </div>
  );
}
...
export function ProfileModal() {
  ...
  const TabBtn = ({ id, label }) => (
    <button
      onClick={() => setTab(id)}
      style={{ padding: "0.45rem 0.85rem", borderRadius: 8, border: "none", background: tab === id ? "var(--accent)" : "transparent", color: tab === id ? "#fff" : "var(--text2)", fontSize: 12.5, cursor: "pointer", fontFamily: "var(--sans)", transition: "all 0.15s", whiteSpace: "nowrap" }}
    >
      {label}
    </button>
  );
    ...
    <div className="modal-backdrop" onClick={() => setModal(null)}>
      <div className="modal-box" style={{ width: "100%", maxWidth: 580 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: "1rem 1.25rem", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0 }}>
          ...
          <button onClick={() => setModal(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text3)", fontSize: 18, lineHeight: 1, marginLeft: "0.5rem" }}>
            ✕
          </button>
        </div>
            ...
            <div>
                  ...
                  {["dark", "light"].map((t) => (
                    <button
                      ...
                      style={{
                        ...
                        background: t === "dark" ? "#09090e" : "#f4f4f8",
                        cursor: "pointer",
                        display: "flex",
              ...
              <Section title="Accent Colour Palette">
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                  {ACCENT_PALETTES.map((pal, idx) => (
                    <button
                      key={pal.name}
                      onClick={() => saveProfile({ paletteIdx: idx })}
                      style={{
                        padding: "0.65rem 0.75rem",
                        borderRadius: 10,
                        border: `2px solid ${profile.paletteIdx === idx ? pal.accent || "var(--accent)" : "var(--border)"}`,
                        background: profile.paletteIdx === idx ? "var(--bg4)" : "var(--bg3)",
                        cursor: "pointer",
                        display: "flex",
                        alignItems: "center",
                        gap: "0.5rem",
                    ...
                    {[
                      ...
                      <div key={key}>
                        ...
                        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                          <input type="color" value={profile[key] || "#7c6dff"} onChange={(e) => saveProfile({ [key]: e.target.value })} style={{ width: 32, height: 28, border: "none", background: "none", cursor: "pointer", padding: 0, borderRadius: 4 }} />
                          <input type="text" value={profile[key] || ""} onChange={(e) => saveProfile({ [key]: e.target.value })} style={{ fontFamily: "var(--mono)", fontSize: 11, flex: 1 }} />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <Section title="Font Family">
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6 }}>
                  {FONT_OPTIONS.map((f, idx) => (
                    <button
                      key={f.name}
                      onClick={() => saveProfile({ fontIdx: idx })}
                      style={{
                        padding: "0.6rem 0.75rem",
                        borderRadius: 9,
                        border: `2px solid ${profile.fontIdx === idx ? "var(--accent)" : "var(--border)"}`,
                        background: "var(--bg3)",
                        cursor: "pointer",
                        fontFamily: f.value,
                        fontSize: 13,
                        color: profile.fontIdx === idx ? "var(--text)" : "var(--text2)",
                        transition: "all 0.15s",
                      }}
                    >
                      {f.name}
              ...
              <Section title="Font Size">
                <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                  ...
                  <button onClick={() => saveProfile({ fontSize: 14 })} style={{ fontSize: 11, color: "var(--text3)", background: "none", border: "1px solid var(--border)", borderRadius: 6, padding: "2px 8px", cursor: "pointer", fontFamily: "var(--sans)" }}>
                    Reset
                  </button>
                </div>
                <div style={{ marginTop: 8, padding: "0.6rem 0.85rem", background: "var(--bg3)", borderRadius: 8, fontSize: profile.fontSize || 14, fontFamily: FONT_OPTIONS[profile.fontIdx]?.value, color: "var(--text2)" }}>The quick brown fox jumps over the lazy dog — 0123456789</div>
              </Section>
            </div>
          )}
          ...
          {tab === "profile" && (
            <div>
              <Section title="Display Name">
                <input type="text" value={profile.displayName} onChange={(e) => saveProfile({ displayName: e.target.value })} placeholder="e.g. André Matias" style={{ fontFamily: "var(--sans)", fontSize: 14 }} />
              ...
              <Section title={profileHash ? "Change Password" : "Set a Password"}>
                {profileHash && !unlocked && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      <button onClick={handleUnlock} style={{ padding: "0.45rem 0.85rem", borderRadius: 8, border: "none", background: "var(--accent)", color: "#fff", cursor: "pointer", fontFamily: "var(--sans)", fontWeight: 600, fontSize: 13 }}>
                        Unlock
                      </button>
                    </div>
                ...
                {(unlocked || !profileHash) && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <div style={{ fontSize: 12, color: "var(--text2)" }}>{profileHash ? "Set a new password:" : "Protect your settings with a password (optional):"}</div>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      {profileHash && (
                        <button
                          ...
                          style={{ padding: "0.45rem 0.85rem", borderRadius: 8, border: "1px solid var(--border)", background: "transparent", color: "var(--text2)", cursor: "pointer", fontFamily: "var(--sans)", fontSize: 13 }}
                        >
                          Remove Password
                        </button>
                      )}
                    </div>
                    <div style={{ fontSize: 11, color: "var(--text3)", lineHeight: 1.5 }}>Password is hashed with SHA-256 and stored locally. ConvertLab never sends data to any server.</div>
                  </div>
                )}
              </Section>
            </div>
          ...
          {tab === "presets" && (
            <div>
              ...
              <Section title="Import Presets">
                <div style={{ fontSize: 13, color: "var(--text2)", marginBottom: "0.75rem", lineHeight: 1.6 }}>Import a previously exported preset file to restore your settings.</div>
                <input
                  ref={importRef}
                  type="file"
                  accept=".json"
