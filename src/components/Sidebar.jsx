...
export default function Sidebar({ activeTool, setActiveTool }) {
  ...
  const favREGISTRY = favourites.map((id) => ALL_TOOLS.find((t) => t.id === id)).filter(Boolean);

  const filteredGroups = [
    ...(favREGISTRY.length > 0 && !q ? [{ section: "Favourites", items: favREGISTRY, isFav: true }] : []),
    ...REGISTRY.map((g) => ({
      ...g,
  ...
  const ToolBtn = ({ tool }) => {
    ...
    return (
      ...
      >
        {/* Icon */}
        <span style={{ width: 22, height: 22, borderRadius: 6, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, background: tool.color + "1a", color: tool.color }}>{tool.icon}</span>
        {/* Label */}
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`sidebar-wrap${sidebarOpen ? "" : " collapsed"}`} style={{ borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", height: "100%", background: "var(--bg)", "--sidebar-w": "224px" }}>
        {/* Top controls */}
        <div style={{ padding: "0.55rem 0.75rem", borderBottom: "1px solid var(--border)", display: "flex", gap: "0.4rem", alignItems: "center", flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: "0.35rem", background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 8, padding: "0.28rem 0.6rem" }}>
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search REGISTRY…" style={{ background: "none", border: "none", outline: "none", fontFamily: "var(--sans)", fontSize: 12, color: "var(--text)", width: "100%", padding: 0 }} />
            {search && (
              <span onClick={() => setSearch("")} style={{ cursor: "pointer", color: "var(--text3)", fontSize: 11, flexShrink: 0 }}>
                ✕
              </span>
            )}
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", paddingBottom: "0.5rem" }}>
          {filteredGroups.length === 0 && q && (
            <div style={{ padding: "1.25rem 1rem", fontSize: 12, color: "var(--text3)" }}>
              No REGISTRY match "<em>{q}</em>"
            </div>
          )}
          ...
          {filteredGroups.map((group) => (
            <div key={group.section}>
              ...
              {group.items.map((tool) => (
                <ToolBtn key={tool.id + group.section} tool={tool} />
              ))}
            </div>
          ))}
        </div>
