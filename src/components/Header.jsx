...
export default function Header({ activeTool, setActiveTool, searchOpen, setSearchOpen, searchInputRef }) {
  ...
  const results = searchVal.trim()
    ? ALL_TOOLS.filter(t =>
        ...
        t.id.includes(searchVal.toLowerCase()))
    : []

  const clearSearch = () => { setSearchVal(''); setSearchOpen(false) }

  const handlePickTool = (id) => { setActiveTool(id); clearSearch() }

  const Chip = ({ icon, title, onClick, badge, label }) => (
    <button
      onClick={onClick}
      title={title}
      style={{ height: 34, padding: '0 10px', borderRadius: 9, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text2)', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, position: 'relative', transition: 'all 0.15s', flexShrink: 0, fontFamily: 'var(--sans)', whiteSpace: 'nowrap' }}
  ...
  return (
    <>
      <header style={{
        ...
        padding: '0 0.85rem', height: 52,
        borderBottom: '1px solid var(--border)',
        background: 'var(--bg-glass)',
        backdropFilter: 'blur(14px)',
        flexShrink: 0, zIndex: 100, gap: '0.5rem',
      }}>

        {/* Logo */}
        <div
          onClick={() => setActiveTool('json')}
          style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', cursor: 'pointer', flexShrink: 0, marginRight: '0.25rem' }}
        >
          ...
          <span style={{ fontFamily: 'var(--display)', fontSize: '1.1rem', fontWeight: 800, letterSpacing: '-0.02em', color: 'var(--text)' }}>
            ConvertLab
          </span>
        </div>
        ...
        <div style={{ flex: 1, maxWidth: 360, position: 'relative' }}>
          ...
          >
            ...
            {searchOpen
              ? <input
                  ...
                  onBlur={() => { if (!searchVal) clearSearch() }}
                  onKeyDown={e => { if (e.key === 'Escape') clearSearch(); if (e.key === 'Enter' && results[0]) handlePickTool(results[0].id) }}
          ...
          {searchOpen && searchVal && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 5, background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 12, boxShadow: 'var(--shadow)', zIndex: 200, overflow: 'hidden', animation: 'slideUp 0.15s ease' }}>
              {results.length === 0
                ? <div style={{ padding: '0.85rem 1rem', fontSize: 12.5, color: 'var(--text3)' }}>No tools match "<em>{searchVal}</em>"</div>
                : results.slice(0, 8).map(tool => (
                  <div key={tool.id} onClick={() => handlePickTool(tool.id)}
