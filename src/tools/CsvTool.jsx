...
export default function CsvTool({ showToast }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      <ToolHeader title="CSV Viewer" desc="Paste CSV and view as interactive table">
        <Btn onClick={() => { navigator.clipboard.writeText(raw); showToast('Copied') }}>Copy</Btn>
      </ToolHeader>
      ...
      <OptionsBar>
        ...
        <OptGroup>
          {DELIMITERS.map(d => (
            <OptBtn key={d.value} active={delim === d.value} onClick={() => setDelim(d.value)}>{d.label}</OptBtn>
          ))}
        </OptGroup>
        {parsed && (
