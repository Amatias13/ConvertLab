...
// ─── Image Tools ──────────────────────────────────────────────────
export function ImageTool({ showToast }) {
  ...
  const onDrop = (e) => { e.preventDefault(); setDrag(false); const f = e.dataTransfer.files[0]; if (f) load(f) }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      <ToolHeader title="Image Tools" desc="Convert, filter and transform images in the browser">
        {img && <>
          ...
          <Btn primary onClick={download}>Download</Btn>
        </>}
      </ToolHeader>
      ...
      {img && (
        <OptionsBar>
          ...
          <OptGroup>
            {[['image/png', 'PNG'], ['image/jpeg', 'JPEG'], ['image/webp', 'WebP']].map(([v, l]) => (
              <OptBtn key={v} active={format === v} onClick={() => setFormat(v)}>{l}</OptBtn>
            ))}
          </OptGroup>
          <OptLabel style={{ marginLeft: '0.75rem' }}>Filter:</OptLabel>
          <OptGroup>
            {[['none', 'None'], ['grayscale', 'Grayscale'], ['sepia', 'Sepia'], ['invert', 'Invert'], ['blur', 'Blur'], ['brightness', 'Bright'], ['contrast', 'Contrast']].map(([v, l]) => (
              <OptBtn key={v} active={filter === v} onClick={() => setFilter(v)}>{l}</OptBtn>
            ))}
          </OptGroup>
      ...
      {!img ? (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
          ...
          <input type="file" ref={fileRef} accept="image/*" style={{ display: 'none' }} onChange={e => { if (e.target.files[0]) load(e.target.files[0]) }} />
        </div>
      ) : (
...
const PRESETS = [
  { name: 'Indigo',   hex: '#7c6dff' }, { name: 'Coral',   hex: '#ff5f7e' },
  { name: 'Mint',     hex: '#3fe8a0' }, { name: 'Sky',      hex: '#38b6ff' },
  { name: 'Amber',    hex: '#ffba3b' }, { name: 'Mauve',    hex: '#a29bfe' },
  { name: 'Teal',     hex: '#00cec9' }, { name: 'Pink',     hex: '#fd79a8' },
  { name: 'Lime',     hex: '#badc58' }, { name: 'Tomato',   hex: '#e55039' },
  { name: 'Navy',     hex: '#2c3e50' }, { name: 'Gold',     hex: '#f9ca24' },
]

function ColorSwatch({ hex, name, onClick }) {
  const [hovered, setHovered] = useState(false)
  return (
    <div onClick={() => onClick(hex)} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid var(--border)', cursor: 'pointer', transform: hovered ? 'scale(1.03)' : 'scale(1)', transition: 'transform 0.15s' }}>
...
export function ColorTool({ showToast }) {
  ...
  const shades = Array.from({ length: 9 }, (_, i) => {
    const t = i / 8
    return rgbToHex(
      Math.round(rgb.r + (255 - rgb.r) * t * 0.85),
      Math.round(rgb.g + (255 - rgb.g) * t * 0.85),
      Math.round(rgb.b + (255 - rgb.b) * t * 0.85),
    )
  })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '0.6rem 1rem', borderBottom: '1px solid var(--border)', display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <input type="text" value={hexInput} onChange={e => { setHexInput(e.target.value); apply(e.target.value) }} style={{ width: 90, fontFamily: 'var(--mono)', fontSize: 13 }} />
        <span style={{ fontSize: 12, color: 'var(--text3)' }}>rgb({rgb.r}, {rgb.g}, {rgb.b})</span>
        <span style={{ fontSize: 12, color: 'var(--text3)' }}>hsl({Math.round(hsl.h)}°, {Math.round(hsl.s)}%, {Math.round(hsl.l)}%)</span>
        <Btn style={{ marginLeft: 'auto' }} onClick={() => { navigator.clipboard.writeText(hex); showToast('Copied: ' + hex) }}>Copy HEX</Btn>
        <Btn onClick={() => { navigator.clipboard.writeText(`rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`); showToast('Copied RGB') }}>Copy RGB</Btn>
      </div>

      <div style={{ flex: 1, overflow: 'auto', padding: '1.25rem' }}>
        <Section title="Shades & Tints">
          {[...shades, ...darks].map((h, i) => <ColorSwatch key={i} hex={h} onClick={apply} />)}
        </Section>
        <Section title="Complementary Palette">
          {palette.map((p, i) => <ColorSwatch key={i} hex={p.hex} name={p.name} onClick={apply} />)}
        </Section>
        <Section title="Preset Swatches">
          {PRESETS.map((p, i) => <ColorSwatch key={i} hex={p.hex} name={p.name} onClick={apply} />)}
        </Section>
      </div>
    </div>
  )
