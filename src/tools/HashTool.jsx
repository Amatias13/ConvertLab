...
export default function HashTool({ showToast }) {
  ...
  useEffect(() => {
    ...
    Promise.all(ALGOS.map(a => hashText(input, a).then(h => [a, h])))
      ...
      <Panels>
        ...
        <Panel>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {ALGOS.length > 0 && input ? ALGOS.map(algo => (
              <div key={algo} style={{
                ...
                onClick={() => { navigator.clipboard.writeText(hashes[algo] || ''); showToast('Copied ' + algo) }}
              >
